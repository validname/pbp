		...
		foreach( $id_draft_array as $temp => $id_draft )
		{
			...
			switch( $temp_array['change_type'])
			{
				case IMPORT_CHANGE_DEL: // delete from transactions
					...
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления имеющихся данных. Экстренный выход."; exit(5); }
				...
				case IMPORT_CHANGE_CHG: // change transactions
					...
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка изменения имеющихся данных. Экстренный выход."; exit(5); }
					$query  = "DELETE FROM import_draft WHERE id_draft=".$id_draft;
					$res2 = db_query($query);
					if( !$res2 )
				...
				default:
					...
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка добавления импортируемых данных - 3. Экстренный выход."; exit(5); }
					$query  = "DELETE FROM import_draft WHERE id_draft=".$id_draft;
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления импортируемых данных - 3. Экстренный выход."; exit(5); }
					break;
			} 			
		}
	}// if( $action=='import' )
