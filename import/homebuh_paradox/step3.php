...
$res = db_query($query);
if( !$res )
	{ echo "<hr>Ошибка запроса названий импортируемых счетов. Экстренный выход."; exit(5); }
...
foreach($imp_account_names as $temp => $imp_account_name )
{
	...
	if( !$silent_mode ) {
		...
		if( !$id_account )
			echo "Без мэппинга счет '".$imp_account_name."'\n";
	}
}

if( $silent_mode ) {
	if( $no_mapping_num ) {
		echo "Без мэппинга: ".$no_mapping_num." счет/ов\n";
		exit(6);
	}
}
else {
