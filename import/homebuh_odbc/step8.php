	...
	if( $action=='import' )
	{
		foreach( $id_draft_array as $temp => $id_draft )
		{
			...
			if( $id_transaction )
			{
				...
				if( !$id_transfer )
					{ echo "<hr>Ошибка связности в имеющихся перемещениях между усчетами (id_transaction=$id_transaction). Экстренный выход."; exit; }
			}
			// обратный мэппинг 
			...
			switch( $temp_array['change_type'])
			{
				case IMPORT_CHANGE_DEL: // delete from transactions
					...
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления имеющихся данных-1. Экстренный выход."; exit; }
					$query  = "DELETE FROM transactions WHERE id_transaction=".$id_transfer;
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления имеющихся данных-2. Экстренный выход."; exit; }
				...
				case IMPORT_CHANGE_CHG: // change transactions
					...
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка изменения имеющихся данных-1. Экстренный выход."; exit; }
					$query  = "UPDATE transactions SET ";
					...
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка изменения имеющихся данных-2. Экстренный выход."; exit; }
					$query  = "DELETE FROM import_draft WHERE id_draft=".$id_draft;
					$res2 = db_query($query);
					if( !$res2 )
				...
				default:
					...
					if( $res2 === false || $res == 0 )
						{ echo "<hr>Ошибка добавления импортируемых данных - 1. Экстренный выход."; exit; }
					$id_transfer1 = $res2; 
					...
					$res2 = db_query($query, true);
					if( $res2 === false || $res == 0 )
						{ echo "<hr>Ошибка добавления импортируемых данных - 2. Экстренный выход."; exit; }
					$id_transfer2 = $res2; 
					$query  = "UPDATE transactions SET id_transfer=".$id_transfer2;
					$query  .= " WHERE id_transaction=".$id_transfer1;
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка изменения имеющихся данных (линковка перемещения). Экстренный выход."; exit; }
					$query  = "DELETE FROM import_draft WHERE id_draft=".$id_draft;
					$res2 = db_query($query);
					if( !$res2 )
