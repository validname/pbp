...
$doc_title = "Шаг ".$current_step.": Соответствие категорий и подкатегорий раcходов";
//$doc_onLoad = "set_mode();";
include("header.php");
...
$query = "SELECT cat, subcat FROM import_draft WHERE id_trans_type=".TRANS_TYPE_EXP." GROUP BY cat, subcat ORDER BY cat, subcat";
$res = db_query($query);
if( !$res )
	{ echo "<hr>Ошибка запроса названий импортируемых категорий. Экстренный выход."; exit; }
...
//	echo "action: ".$action."<hr>";
	...
	{// if( !$updating_invalid_value )
		if( $action=='set_mapping' )
		{
			...
			$query_where .= " AND id_trans_type=".TRANS_TYPE_EXP." AND id_import_mod=".$id_import_mod; 
			...
			{//if( $result )
				...
				if( $inserting )
				{
					...
					$db_query .= $id_import_mod.", ".TRANS_TYPE_EXP.", '".db_add_slashes($form_cat_name)."', ";
...
$query .= "WHERE id_import_mod=".$id_import_mod." AND id_trans_type=".TRANS_TYPE_EXP." ORDER BY cat, subcat";
$res = db_query($query);
if( !$res )
