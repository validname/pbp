...
if( $last_step === false )
	{ echo "<hr>Ошибка чтения конфигурационных данных. Экстренный выход."; exit; }

$doc_title = "Шаг ".$current_step.": Перенос доходов в БД";
//$doc_onLoad = "set_mode();";
include("header.php");
...
$query .= "WHERE id_import_mod=".$id_import_mod." AND id_trans_type=".TRANS_TYPE_INC." ORDER BY cat, subcat, subcat2";
$res = db_query($query);
if( !$res )
	{ echo "<hr>Ошибка запроса мэппинга категорий. Экстренный выход."; exit; }
while( $temp_array = db_fetch_num_array($res) )
{
		...
		foreach( $id_draft_array as $temp => $id_draft )
		{
			...
			switch( $temp_array['change_type'])
			{
				case IMPORT_CHANGE_DEL: // delete from transactions
					...
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления имеющихся данных. Экстренный выход."; exit; }
					$query  = "DELETE FROM import_draft WHERE id_draft=".$id_draft;
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления импортируемых данных - 1. Экстренный выход."; exit; }
					break;
				case IMPORT_CHANGE_CHG: // change transactions
					...
					$query  .= " value='".$value."', "; // plus for incomes
					$query  .= " comment='".db_add_slashes($comment)."' ";
					$query  .= " WHERE id_transaction=".$id_transaction;
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка изменения имеющихся данных. Экстренный выход."; exit; }
					$query  = "DELETE FROM import_draft WHERE id_draft=".$id_draft;
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления импортируемых данных - 2. Экстренный выход."; exit; }
					break;
				...
				default:
					...
					$query  .= $id_subcat2.", ".$id_account.", '".$date."', ".$quantity.", '".$value."', "; // plus for incomes
					$query  .= "'".db_add_slashes($comment)."') ";
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка добавления импортируемых данных - 3. Экстренный выход."; exit; }
					$query  = "DELETE FROM import_draft WHERE id_draft=".$id_draft;
					$res2 = db_query($query);
					if( !$res2 )
						{ echo "<hr>Ошибка удаления импортируемых данных - 3. Экстренный выход."; exit; }
					break;
			} 			
		}
	}// if( $action=='import' )
...
{ //if( !$action )
	...
	if( $last_step>$current_step )
		{ echo "<hr>Вы искусственно вернулись на этот шаг назад после выполнения необратимых шагов. Лучше <a href=\"../step1.php\">начать импорт данных сначала</a>."; exit; }	
	...
	if( $last_step<$current_step )
	{
		...
		$res = db_query($query);
		if( !$res )
			{ echo "<hr>Ошибка удаления лишних (по датам) импортируемых данных. Экстренный выход."; exit; }
		...
		$res = db_query($query);
		if( !$res )
			{ echo "<hr>Ошибка запроса данных о транзакциях. Экстренный выход."; exit; }
...
//		print_r($data_array);
		...
		$query .= "WHERE id_trans_type=".TRANS_TYPE_INC." AND change_type='".IMPORT_CHANGE_ADD."' ORDER BY date";
		$res = db_query($query);
		if( !$res )
			{ echo "<hr>Ошибка запроса импортируемых данных. Экстренный выход."; exit; }
...
//temp-dps
			$res = db_query($query);
			if( !$res )
				{ echo "<hr>Ошибка удаления совпадающих импортируемых данных. Экстренный выход."; exit; }
		}
...
//temp-dps
								$res = db_query($query);
								if( !$res )
									{ echo "<hr>Ошибка изменения измененных импортируемых данных. Экстренный выход."; exit; }
								unset($data_array[$index][$temp]);
								$changed++;
								break;
						}
...
//		print_r($import_array);
		...
		foreach( $data_array as $index => $data2 )
		{
			foreach( $data2 as $temp => $data )
			{
				...
				$query .= TRANS_TYPE_INC.", ";	// расходы
				$query .= "\"".IMPORT_CHANGE_DEL."\", ";
				$query .= $data['id_transaction'].")";
//temp-dps				
				$res = db_query($query);
				if( !$res )
					{ echo "<hr>Ошибка занесения удаляемых данных. Экстренный выход."; exit; }
			}
		}
		echo "<hr><br>\r\n";
		flush();
...
$query .= "WHERE id_trans_type=".TRANS_TYPE_INC;
if( $query_order )
...
// 6. Вывод формы
function echo_field($field_name, $field_text, $sort_field, $sort_dsc)
{
	echo "<th class=list id=framed>";
	if( $sort_field==$field_name )
	{ 
		echo "<a href=".$_SERVER['PHP_SELF']."?sort_field=".$field_name;
		if( $sort_dsc ) echo ">";
		else	echo "&sort_dsc=1>";
	}
	else
		echo "<a href=".$_SERVER['PHP_SELF']."?sort_field=".$field_name.">"; 
	echo $field_text;
